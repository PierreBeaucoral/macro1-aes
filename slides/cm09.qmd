...
## Le diagnostic conditionne la politique active {.smaller}
...
::: {.source}
Source : d'après Bénassy-Quéré, Coeuré, Jacquet, Pisani-Ferry (2021) [@benassy2021].
:::
...
ggplot(df_epl, aes(x = pays, y = valeur, fill = indicateur)) +
  ...
  # Fait marquant du fig-cap : l'écart Danemark/France se creuse surtout
  # sur l'EPC (licenciements collectifs), pas sur l'EPRC.
  annotate("segment", x = 3.18, xend = 4.18, y = 3.55, yend = 3.55,
           arrow = arrow(ends = "both", length = unit(.12, "cm")),
           color = unname(macro_pal["rouge"]), linewidth = .5) +
  annotate("text", x = 3.68, y = 3.85, label = "+1,07 point sur l'EPC",
           size = 3, color = unname(macro_pal["rouge"]), fontface = "italic") +
...
plot_triangle <- function(k) {
  ...
  p +
    geom_point(data = sommets_k, aes(x, y), size = 3.5, color = macro_pal["bleu"]) +
...
etiquettes_ch <- df_chomage[df_chomage$annee == max(annees), ]

ggplot(df_chomage, aes(x = annee, y = taux, color = pays, linewidth = pays)) +
  geom_line() +
  geom_text(data = etiquettes_ch, aes(label = pays), hjust = 0, nudge_x = 0.6,
...
## Quatre notions de sécurité à ne pas confondre {.smaller}
...
::: {.source}
Sources : Chapitre III, Section I (`cours/chap3.qmd`) ; Bénassy-Quéré, Coeuré, Jacquet, Pisani-Ferry (2021) [@benassy2021].
:::
