...
ggplot(world) +
  ...
  scale_fill_gradient(low = "#fbe3c8", high = unname(macro_pal["rouge"]),
...
cols_wsps <- c("WS : fixation des salaires" = macro_pal[["bleu"]],
               "PS : fixation des prix (m = 0,43)" = macro_pal[["or"]],
               "PS après hausse de m (m = 0,50)" = macro_pal[["rouge"]])
...
plot_wsps <- function(k) {
  ...
  if (k >= 2) {
    p <- p + geom_line(data = df_ps1, aes(x = u, y = w, color = "PS : fixation des prix (m = 0,43)"),
                        linewidth = 1.1)
  }
  ...
  if (k == 4) {
    p <- p +
      ...
      geom_line(data = df_ps2, aes(x = u, y = w, color = "PS après hausse de m (m = 0,50)"), linewidth = 1.1) +
  ...
  p +
    scale_color_manual(values = cols_wsps, name = NULL, breaks = names(cols_wsps)) +
