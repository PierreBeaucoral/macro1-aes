...
ggplot() +
  ...
  geom_curve(data = fleches[3, ], aes(x = x, y = y, xend = xend, yend = yend),
             curvature = 0.25, arrow = arrow(length = unit(0.22, "cm")),
             color = macro_pal["gris"], linewidth = 0.6) +
  geom_point(data = sommets, aes(x, y), size = 3.5, color = macro_pal["bleu"]) +
  geom_label(data = sommets, aes(x, y, label = label),
             nudge_y = c(-0.09, -0.09, 0.11),
             size = 3, fontface = "bold", color = "#1f4e79",
...
## Chaque axe cible un diagnostic théorique du chapitre II {.smaller}
...
ggplot(reformes, aes(x = annee, y = 0, color = axe)) +
  ...
  geom_point(size = 2.4) +
  # Repere recalibre pour la hauteur reduite (4.6 po) : texte, décalage et
  # marge plus serres, force de repulsion un peu plus forte pour compenser
  # l'espace vertical disponible en moins.
  geom_text_repel(aes(label = label), size = 2.15, lineheight = 0.8, show.legend = FALSE,
                   direction = "y", nudge_y = 0.4, segment.size = 0.3,
                   segment.color = "grey60", box.padding = 0.1, force = 3.5,
...
plot_trappe <- function(k) {
  ...
  # Étiquette directe en bout de courbe plutôt qu'une légende : nom court
  # du régime, avec un décalage vertical propre à chaque régime pour éviter
  # que RSA et prime d'activité ne se chevauchent en fin de tracé.
  lab_trappe <- d[d$x == max(d$x), ]
  noms_courts <- c("RMI (avant 2009)" = "RMI",
                    "RSA (depuis 2009)" = "RSA",
                    "+ Prime d'activité (depuis 2016)" = "Prime d'activité")
  decalages   <- c("RMI (avant 2009)" = 0,
                    "RSA (depuis 2009)" = -4,
                    "+ Prime d'activité (depuis 2016)" = 4)
  lab_trappe$nom  <- noms_courts[as.character(lab_trappe$regime)]
  lab_trappe$yfin <- lab_trappe$y + decalages[as.character(lab_trappe$regime)]

  p <- ggplot() +
    geom_abline(intercept = 100, slope = 0, linetype = "dotted",
                color = "grey70", linewidth = 0.4) +
    geom_line(data = d, aes(x = x, y = y, color = regime), linewidth = 1.1) +
    geom_text(data = lab_trappe, aes(x = x, y = yfin, label = nom, color = regime),
              hjust = 0, nudge_x = 3, size = 3.2, fontface = "bold", show.legend = FALSE)
  ...
  p +
    ...
    labs(x = "Revenu d'activité (% du SMIC net travaillé)",
         y = "Revenu disponible (indice, base 100)") +
    theme_macro() +
...
ggplot(seniors, aes(x = annee, y = taux, color = pays)) +
  ...
  labs(x = NULL, y = "Taux d'emploi des 55-64 ans (%)") +
  theme_macro() +
...
etiquettes_cmp <- df_cmp[df_cmp$annee == max(annees_cmp), ]

ggplot(df_cmp, aes(x = annee, y = taux, color = pays, linewidth = pays)) +
  geom_line() +
  geom_text(data = etiquettes_cmp, aes(label = pays), hjust = 0, nudge_x = 0.6,
  ...
  scale_linewidth_manual(values = c("France" = 1.1, "Danemark" = 1.1, "États-Unis" = 0.7),
                          guide = "none") +
  scale_x_continuous(breaks = seq(2000, 2024, 4), limits = c(2000, 2032)) +
...
lab_cdd <- df_cdd[df_cdd$annee == max(annees_cdd), ]

ggplot(df_cdd, aes(x = annee, y = valeur, color = mesure)) +
  geom_line(linewidth = 1.1) +
  geom_point(size = 1.8) +
  geom_text(data = lab_cdd, aes(label = paste0(valeur, " %")), hjust = 1.15,
...
## Quatre regards complémentaires sur le chômage (rappel Chapitre II) {.smaller}
...
# Seuls les trois pays commentés dans le fig-cap (France, Allemagne, Suède)
# sont étiquetés, et seulement sur la tranche 60-64 ans qui porte le
# propos : pas de valeur sur chacune des douze barres.
lab_age <- subset(df_age, tranche == "60-64 ans" & pays %in% c("France", "Allemagne", "Suède"))
decalages_age <- c(France = -0.3, UE27 = -0.1, Allemagne = 0.1, Suède = 0.3)
lab_age$x_pos <- 3 + decalages_age[as.character(lab_age$pays)]

ggplot(df_age, aes(x = tranche, y = taux, fill = pays)) +
  geom_col(position = position_dodge(width = .8), width = .72) +
  geom_text(data = lab_age, aes(x = x_pos, y = taux, label = paste0(taux, " %")),
            vjust = -0.5, size = 3.2, fontface = "bold", color = "#1f4e79",
            inherit.aes = FALSE) +
...
::: {.source}
Sources : Chapitre III, §II (`cours/chap3.qmd`) ; DARES (2016), *Faut-il baisser le coût du travail ?* ; *Prestations sociales et trappe à inactivité* ; *Réformes du droit du travail en France* ; *Protection de l'emploi, emploi et chômage* ; Bénassy-Quéré et al. (2021) [@benassy2021].
:::
